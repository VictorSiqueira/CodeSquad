...
//por estarem se relacionando por outro modulo, não há necessidade de declarar o modulo tarefas aqui
...
Usuarios.controller('UsuariosController', ['$scope', 'Usuarios', '$location','$routeParams', function ($scope, $Usuarios, $location, $routeParams) {
    $scope.usuarios = $Usuarios.query();
    $scope.novo = function () {
    	Usuario.save(
    			{}, //model id - identifica a ocorrencia do banco a ser mexida
    			$scope.usuarios, //dados a serem salvos
    			function (){ // o primeiro fucntion é sucess
    				$scope.usuarios = ''; //zerar a model para ficar livre receber novos dados caso ele clicque em cancelar da linha de baixo 
    ...
    $scope.ver = function () {
    	$scope.usuarios = $Usuario.get({id:$routeParams.id});
    }
